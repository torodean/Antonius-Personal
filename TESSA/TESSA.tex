...
#define PUMPPIN1 10 /*Pump 1 signal pin - SIDE PUMP*/
#define PUMPPIN2 4  /*Pump 2 signal pin - TOP PUMP*/
...
long delayTime = 24L*60L*60L - onTime1 - onTime2; // one day - on times.
long count = delayTime - 60;


void loop() {
  delay(1000);
  count++;
  timer = delayTime - count;
  if (count >= delayTime + onTime1 + onTime2) {
    digitalWrite(PUMPPIN2, LOW);
    displayText(false);
    count = 0;
  } else if(count >= delayTime + onTime1){
...
void displayText(bool pumpStatus){
  ...
  if (pumpStatus) {
    ...
    activeTimer = onTime1 + onTime2 + timer;
    display.print(activeTimer);
